...
class AuthMutation:
    ...
    @strawberry.mutation
    async def register(self, input: RegisterInput, info: Info) -> AuthPayload:
        """
        Register a new user.
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                existing_user = await UserService.get_user_by_email(db, input.email)
                if existing_user:
                    raise UserAlreadyExistsError(field="email")
                
                # Check if username already exists
                existing_username = await UserService.get_user_by_username(db, input.username)
                if existing_username:
                    raise UserAlreadyExistsError(field="username")
                
                # Check if there are pending invitations for this email
                pending_invites = await TeamService.get_pending_invitations_for_email(db, input.email)
    ...
    @strawberry.mutation
    async def login(self, input: LoginInput, info: Info) -> AuthPayload:
        """
        Authenticate a user.
        ...
        async with AsyncSessionLocal() as db:
            try:
                # Authenticate user
                user = await UserService.authenticate_user(db, input.email, input.password)
                if not user:
    ...
    @strawberry.mutation
    async def request_password_reset(self, input: RequestPasswordResetInput, info: Info) -> PasswordResetResult:
        """
        ...
        async with AsyncSessionLocal() as db:
            try:
                # Find user by email
                user = await UserService.get_user_by_email(db, input.email)
                
                if user and user.is_active:
                    # Create reset token
                    reset_token = await UserService.create_password_reset_token(db, user)
    ...
    @strawberry.mutation
    async def reset_password(self, input: ResetPasswordInput, info: Info) -> PasswordResetResult:
        """
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                success = await UserService.reset_password(db, input.token, input.new_password)
...
class ProfileMutation:
    ...
    @strawberry.mutation
    async def update_profile(self, input: UpdateProfileInput, info: Info) -> ProfileUpdateResult:
        """
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if input.email and input.email != user.email:
                    existing = await UserService.get_user_by_email(db, input.email)
                    if existing:
                ...
                if input.username and input.username != user.username:
                    existing = await UserService.get_user_by_username(db, input.username)
                    if existing:
                        return ProfileUpdateResult(
                            success=False,
    ...
    @strawberry.mutation
    async def change_password(self, input: ChangePasswordInput, info: Info) -> ProfileUpdateResult:
        """
...
class OnboardingMutation:
    ...
    @strawberry.mutation
    async def complete_onboarding(self, info: Info) -> UserType:
        """
